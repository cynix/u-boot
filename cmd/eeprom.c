...
static int eeprom_rw_block(unsigned offset, uchar *addr, unsigned alen,
			   uchar *buffer, unsigned len, bool read, int i2c_bus)
{
	int ret = 0;
...
static int eeprom_rw(unsigned dev_addr, unsigned offset, uchar *buffer,
		     unsigned cnt, bool read, int i2c_bus)
{
	...
	while (offset < end) {
		...
		rcode = eeprom_rw_block(offset, addr, alen, buffer, len, read, i2c_bus);
...
{
	return eeprom_read_i2c(-1, dev_addr, offset, buffer, cnt);
}

int eeprom_read_i2c(int i2c_bus, unsigned dev_addr, unsigned offset,
		uchar *buffer, unsigned cnt)
{
	 ...
	 */
	return eeprom_rw(dev_addr, offset, buffer, cnt, 1, i2c_bus);
}

int eeprom_write(unsigned dev_addr, unsigned offset,
		 uchar *buffer, unsigned cnt)
{
	return eeprom_write_i2c(-1, dev_addr, offset, buffer, cnt);
}

int eeprom_write_i2c(int i2c_bus, unsigned dev_addr, unsigned offset,
		 uchar *buffer, unsigned cnt)
{
	int ret;
	 ...
	 */
	ret = eeprom_rw(dev_addr, offset, buffer, cnt, 0, i2c_bus);
				  ...
				  char *value, ulong addr, ulong off, ulong cnt)
{
	...
	if (action == EEPROM_READ) {
		...
		rcode = eeprom_read_i2c(-2, i2c_addr, off, (uchar *)addr, cnt);
		...
		rcode = eeprom_write_i2c(-2, i2c_addr, off, (uchar *)addr, cnt);
...
#ifdef CONFIG_CMD_EEPROM_LAYOUT
	rcode = eeprom_read_i2c(-2, i2c_addr, 0, eeprom_buf, CONFIG_SYS_EEPROM_SIZE);
	if (rcode < 0)
		return rcode;
	...
	rcode = eeprom_write_i2c(-2, i2c_addr, 0, layout.data, CONFIG_SYS_EEPROM_SIZE);
#endif
