...
{
	int i;
	const struct eeprom_field *fields = layout->fields;
	const unsigned char *fbuf = layout->data;
				      ...
				      char *field_name, char *new_data)
{
	...
	unsigned char *fbuf = layout->data;
...
/*
 * eeprom_layout_read_field() - read a single field in the layout data.
 * @layout:	A pointer to an existing struct layout.
 * @field_name:	The name of the field to read.
 * @buf:	Buffer to hold the field's value.
 *
 * Returns: 0 on success, negative error value on failure.
 */
static int eeprom_layout_read_field(const struct eeprom_layout *layout,
				      char *field_name, unsigned char *buf)
{
	int i;
	const struct eeprom_field *fields = layout->fields;
	const unsigned char *fbuf = layout->data;
	...
	for (i = 0; i < layout->num_of_fields; fbuf += fields[i++].size) {
		...
		fields[i].read(&fields[i], fbuf, buf);
